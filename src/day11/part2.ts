...
for (const def of input) {
    ...
    let modulo = parseInt(lines[3].split('by ')[1]);
    product *= modulo;
    let test = (a: number) => (a % modulo == 0);
...
for (let i = 1; i <= 10000; i++) {     // 10000 rounds of inspecting and throwing
    for (const monkey of monkeys) {
        while (monkey.items) {
            let num = monkey.items.shift();
            if (num == null) break;
            let newNum = monkey.operation(num) % product;
