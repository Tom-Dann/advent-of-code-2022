...
// Opened is number representing current valves opened stored in each bit (0->2^14)
let findSolution = (time: number, current: number, opened: number, pressure: number) => {
    ...
    if (pressure > best.pressure) best = {pressure, opened};
}

findSolution(26, 0, 0, 0);
findSolution(26, 0, best.opened, best.pressure);
console.log('Answer:', best.pressure);